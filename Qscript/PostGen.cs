...
namespace Qscript
{
    ...
    public static class PostGen
    {
        ...
        public static objProgram PostTranslation(objProgram _objProgram, ProgramNode _ProgramAst)
        {
            ...
            List<inst> macroList = LexInstructs(RegisterMachine(LexInstructs(_objProgram.macroData), true));
            objProgramResualt.macroData = IndicatorsCorrection(macroList, varGlobal);
        ...
        public static StringData RegisterMachine(List<inst> instructs, bool line, Dictionary<string, string> functionRegisterData=null)
        {
            ...
            string getFreeReg (string reg)
            {
                ...
                else if (reg.EndsWith("word") || reg.EndsWith("byte"))
                {
                    ...
                    for (int i = 0; i < asmRegisters.Length; i++)
                    {
                        if (regType == "byte" && (asmRegisters[i] == "esi" || asmRegisters[i] == "edi")) continue;
                        if (!tableRegisters.ContainsValue(asmRegisters[i])) regReturn = asmRegisters[i];
                    }
                    if (regReturn == string.Empty) regReturn = asmRegisters[rand.Next(0, 5)];
        ...
        public static StringData IndicatorsCorrection(List<inst> instructs, Dictionary<string, string> vars)
        {
            ...
            for (int i = 0; i < instructs.Count; i++)
            {
                try
                {
                    ...
                    // local vars
                    // // local Name dd 0
                    if (_inst.value == "local" || _inst.value == "mlocal")
                    {
                        ...
                        if (_inst.value[0] != 'm') resualt.Append(resualtLocalString);
                        continue;
                    }
                ...
                {
                    resualt.Append(instructs[i].ToString());
                }
            }
        ...
        public static StringData Translation (List<inst> instructs, Dictionary<string, string> varGlobal)
        {
            ...
            for (int i = 0; i < instructs.Count; i++)
            {
                ...
                try
                {
                    ...
                    // |case3| - cmp
                    if (InstructPattern(str, "mov|rn") && InstructPattern(str2, "cmp|rm") && InstructCmpReg(pattern1, pattern2))
                    {
